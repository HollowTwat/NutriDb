    ...
    public class TypesCRUDController : Controller
    {
        ...
        public ActionResult<GetMealResponse> GetTodayUserMeals(long userTgId)
        {
            try
            {
                ...
                var meals = _context.Meals.Where(x => x.UserId == user.Id && DateTime.Equals(x.MealTime.Date, DateTime.UtcNow.AddHours(3).Date)).ToList();
        ...
        public async Task<ActionResult<List<GetWeekMealStatusResponse>>> GetUserWeekMealsStatus(long userTgId)
        {
            try
            {
                ...
                var startDate = DateTime.UtcNow.AddHours(3).AddDays(-7).Date;
        ...
        public async Task<ActionResult<GetMealTotalResponse>> GetUserMealsTotal(long userTgId, Periods period)
        {
            try
            {
                ...
                DateTime startDate = DateTime.UtcNow.AddHours(3).Date;
                int daysinperiod = 0;
                var now = DateTime.UtcNow.AddHours(3).Date;
                switch (period)
                {
                    case Periods.day:
                        startDate = DateTime.UtcNow.AddHours(3).AddDays(-1).Date;
                        break;
                    case Periods.week:
                        startDate = MealHelper.GetFirstDayOfWeek(now);
                        break;
                    case Periods.mathweek:
                        startDate = DateTime.UtcNow.AddHours(3).AddDays(-7).Date;
                        break;
                    case Periods.math3weeks:
                        startDate = DateTime.UtcNow.AddHours(3).AddDays(-21).Date;
                        break;
                    case Periods.month:
        ...
        public async Task<ActionResult<GetMealResponse>> EnsureUser(long userTgId, string userName, long userNoId)
        {
            try
            {
                ...
                _context.Users.Add(new DbModels.User
                {
                    ...
                    StageId = 0,
                    LessonId = 0,
                    RegistrationTime = DateOnly.FromDateTime(DateTime.UtcNow.AddHours(3)),
                    IsActive = true,
        ...
        public async Task<ActionResult<bool>> GetUserWeekPlot(long userTgId)
        {
            try
            {
                ...
                DateTime startDate = DateTime.UtcNow.AddHours(3).AddDays(-7).Date;
                int daysinperiod = 0;
                var now = DateTime.UtcNow.AddHours(3).Date;
            ...
            try
            {
                ...
                if (usi == null)
                {
                    await _context.Userinfos.AddAsync(new Userinfo
                    {
                        ...
                        LastlessonTime = DateTime.UtcNow.AddHours(3),
                    });
                }
                else
                {
                    usi.LastlessonTime = DateTime.UtcNow.AddHours(3);
        ...
        public async Task<ActionResult<GetUserPingResponse>> GetUserPing(long UserTgId, short TimeOfDay)
        {
            try
            {
                ...
                DateTime ping = DateTime.UtcNow.AddHours(3);
                if (TimeOfDay == 0)//утро
                {
