...
namespace NutriDbService.Helpers
{
    ...
    public class TransmitterHelper
    {
        ...
        {
            var lastreqid = 0;
            var req = new Gptrequest { Iserror = false, Request = JsonConvert.SerializeObject(request), Done = false, UserTgid = request.UserTgId, CreationDate = DateTime.UtcNow.AddHours(3), ReqType = string.IsNullOrEmpty(request?.Type) ? "empty" : request.Type };
            bool send = true;
        ...
        public async Task<bool> ExecuteRequest(CreateGPTPythRequest reqparams, string reqUrl, int requstId)
        {
            ...
            catch (Exception ex)
            {
                ...
                using (var scope = _serviceProviderFactory.CreateScope().ServiceProvider.CreateScope())
                {
                    ...
                    dbreq.FinishDate = DateTime.UtcNow.AddHours(3);
                    dbreq.Done = true;
